 ...
 */
public class RustBeaconHashSidechainTest {
    ...
    private static final SimpleLog log = new SimpleLog(RustBeaconHashSidechainTest.class.getName());

    public RustBeaconHashSidechainTest() {
        manager1 = new BeaconHashSidechainNodeManager(NodeType.RUST_NODE,
            System.getProperty("user.dir") + "/aionr",
            System.getProperty("user.dir") + "/test_resources/rust_custom1"
        );

        manager2 = new BeaconHashSidechainNodeManager(NodeType.RUST_NODE,
            System.getProperty("user.dir") + "/aionr2",
            System.getProperty("user.dir") + "/test_resources/rust_custom2"
        );
    }
    ...
    public void test() throws Exception {
        ...
        minedBlocks.putAll(awaitBlockSealedAndCapture(listener2, 6));
        ...
        minedBlocks.putAll(awaitBlockSealedAndCapture(listener2, 12));
        ...
        awaitBlockSealedAndCapture(listener1, 9);
...
//        manager2.startLocalNode(false, false);
        ...
        // seem to ever happen
        List<BlockImportMessage> bims = awaitAndCaptureImportStatus(listener1, minedBlocks, manager2,reorg_num);
        ...
        assertThat("block with the transaction should have been successfully imported as a side chain block",
            importedTxBlockResult, is("success"));
        String is_true = "false";
        if (txBim.get(0).number >= reorg_num){
            is_true = "true";
        }
        assertThat("block with the transaction should have been successfully imported as a side chain block",
                is_true, is("true"));
    ...
    private static final Pattern REORG_NUMBER_CAPTURE = Pattern.compile(
            "Reorg to #(\\d+)");
    ...
    awaitAndCaptureImportStatus(NodeListener listener, Map<Long, String> minedBlocks, BeaconHashSidechainNodeManager mgr2, Long reorg_num)
    throws Exception {
        List<FutureResult<LogEventResult>> expectedFutures = new LinkedList<>();
        for(Long bn : minedBlocks.keySet()) {
            IEvent blockImportedEv = new Event("New block #" + bn + ", hash:" );
        ...
        IEvent blockImportedEv = new Event("Reorg to #");
        FutureResult<LogEventResult> result = listener.listenForEvent(blockImportedEv,3,TimeUnit.MINUTES);

        mgr2.startLocalNode(false,false);
        log.log("Start to litsen...");
        ...
        for(FutureResult<LogEventResult> resultFuture : expectedFutures) {
            ...
            m.find();
            BlockImportMessage bim = new BlockImportMessage(m.group(2), Long.parseLong(m.group(1)), "success");
            log.log("Saw import-status message: " + bim.toString());
            capturedMsgs.add(bim);
        }
        LogEventResult res = result.get(3,TimeUnit.MINUTES);
        String line = res.getObservedLogs().get(0);
        Matcher m = REORG_NUMBER_CAPTURE.matcher(line);
        m.find();
        reorg_num = Long.parseLong(m.group(1));
    ...
    private static final Pattern BLOCK_SEALED_NUM_CAPTURE = Pattern.compile(
        "Local Po[SW] block added\\. #(\\d+), hash: (\\w+),");
    ...
    throws Exception {
        ...
        while(mostRecentBlockNumberSeen < number) {
            IEvent blockSealedEv = new Event("block added. #");
            FutureResult<LogEventResult> resultFuture =
                listener.listenForEvent(blockSealedEv, 8, TimeUnit.MINUTES);

            LogEventResult res = resultFuture.get(10, TimeUnit.MINUTES);
            if(! res.eventWasObserved()) {
